...
var i; const unknowns= [];
for (i = 0; i < chapters.data.length; i++) {
  var book = new Option(chapters.data[i].name, i);
  book.value = chapters.data[i].id.toLowerCase();
  if (i < 39 || i > 52) {
    bibleBooks.options.add(book);
  } else {
    unknowns.push(chapters.data[i].id);
  }
}
