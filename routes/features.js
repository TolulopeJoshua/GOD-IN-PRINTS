...
const {getImage, encode} = require('../functions');
...
router.get('/', async (req, res) => {
    ...
    // const bioKey = myBio.image.key;
    // const bioImg = await getImage(bioKey);
    // const bioImage="<img class='card-img-top' width='640' alt='' src='data:image/jpeg;base64," + encode(bioImg.Body) + "'" + "/>";
    

    const book = await Book.aggregate( [ { $sample: { size: 1 } } ] ); const myBook = book[0];
    // const key = myBook.image.key;
    // const img = await getImage(key);
    // const image="<img class='card-img-top img-thumbnail' width='640' alt='' src='data:image/jpeg;base64," + encode(img.Body) + "'" + "/>";
